...
void bubble_sort(int int_arr[], size_t length) {
    for (size_t index = 0; index < length - 1; index++) {
        bool is_ordered = true;

        for (size_t j = 0; j < length - 1; j++) {
            int int_current = int_arr[j];
            int int_next = int_arr[j + 1];
...
int main() {
    ...
    bubble_sort(test_1_arr, 5);
    if (!is_arr_equal(test_1_arr, arr_sorted, 5)) {
        println("Test 1 failed");
        print_array<int>("Received", test_1_arr, 5);
        print_array<int>("Expected", arr_sorted, 5);
    }

    int test_2_arr[] = { 5, 4, 3, 2, 1 };
    bubble_sort(test_2_arr, 5);
    if (!is_arr_equal(test_2_arr, arr_sorted, 5)) {
        println("Test 2 failed");
        print_array<int>("Received", test_2_arr, 5);
        print_array<int>("Expected", arr_sorted, 5);
    }

    int test_3_arr[] = { 1, 3, 2, 5, 4 };
    bubble_sort(test_3_arr, 5);
    if (!is_arr_equal(test_3_arr, arr_sorted, 5)) {
        println("Test 3 failed");
        print_array<int>("Received", test_3_arr, 5);
        print_array<int>("Expected", arr_sorted, 5);
    }

    int test_4_arr[] = { 5, -1, 3, 2, 4, -3, -5, -6, 10, -20 };
    int test_4_arr_sorted[] = { -20, -6, -5, -3, -1, 2, 3, 4, 5, 10 };
    bubble_sort(test_4_arr, 10);
    if (!is_arr_equal(test_4_arr, test_4_arr_sorted, 10)) {
        println("Test 4 failed");
        print_array<int>("Received", test_4_arr, 10);
        print_array<int>("Expected", test_4_arr_sorted, 10);
    }
